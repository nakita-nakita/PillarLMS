// libraries
import React, { useContext } from 'react'

// mine
import InformationModal from '@/components/modals/Information.modal';
import { SiteDesignerPageSectionContext } from '../context/SiteDesignerPageSection.context';

//mui
import TextField from '@mui/material/TextField';


function NormalSectionDeletionModal({ isOpened, onClose, onSubmit }) {
  const { nameInput, setNameInput, name, } = useContext(SiteDesignerPageSectionContext)

  const handleSubmit = () => {
    if (onSubmit) {
      onSubmit()
    }

    if (onClose) {
      onClose()
    }
  }

  return (
    <InformationModal
      isOpened={isOpened}
      onClose={onClose}
      header="Delete Section."
      onSubmit={handleSubmit}
      submitLabel={"Delete Section"}
      disableSubmit={name !== nameInput}
    >
      <p>Please enter the section name "{name}" to delete it.</p>
      <br />
      <TextField
        id="outlined-basic"
        label="Section Name"
        variant="outlined"
        fullWidth
        value={nameInput}
        onChange={(event) => setNameInput(event.target.value)}
      />
    </InformationModal>
  )
}

// MeetingChangeNameModal.propTypes = {
//   isOpened: PropTypes.boolean,
//   onClose: PropTypes.func,
// }

export default NormalSectionDeletionModal
