...
import { getSettingSiteGraphQL } from '../store/settingSite_getOneRealTime.store';

export const SettingSiteContext = React.createContext();

export function SettingSiteProvider({ children }) {
  ...
  useEffect(() => {
    ...
    getSettingSiteGraphQL({
      socketId,
    }).then(result => {
      const site = result.data.backendSettingSite_getOneRealTime
  ...
  return (
    <SettingSiteContext.Provider value={{
      isLoaded, setLoaded,
      id, setId,
      entity, setEntity,
      favicon, setFavicon,
      ...
      {children}
    </SettingSiteContext.Provider>
  )
}

export default SettingSiteProvider