...
const config = {
  HOST: process.env.POSTGRES_DOMAIN_TEST_HOST,
  USER: process.env.POSTGRES_DOMAIN_TEST_USER,
  PASSWORD: process.env.POSTGRES_DOMAIN_TEST_PASSWORD,
  DB: process.env.POSTGRES_DOMAIN_TEST_DB,
  dialect: "postgres",
  pool: {
    max: parseInt(process.env.POSTGRES_DOMAIN_TEST_POOL_MAX),
    min: parseInt(process.env.POSTGRES_DOMAIN_TEST_POOL_MIN),
    // acquire: parseInt(process.env.POSTGRES_TEST_POOL_ACQUIRE),
    idle: parseInt(process.env.POSTGRES_DOMAIN_TEST_POOL_IDLE)
  }
}
...
export default async function connectToDomainTestDb(): Promise<Sequelize> {
