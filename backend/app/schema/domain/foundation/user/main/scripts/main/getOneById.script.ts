  ...
  return async (args: input): Promise<returningSuccessObj<Model<foundationUser> | null>> => {
    ...
    if (!args.id) {
      return endMainFromError({
        ...
        errorIdentifier: "foundationUser_getOneById_error0001"
      })
    }
    ...
    if (!isIdStringFromUuid.result) {
      return endMainFromError({
        hint: "Datapoint 'id' is not UUID format.",
        errorIdentifier: "foundationUser_getOneById_error0002"
      })
    }
    ...
    const response = await foundationUserSql.getOneById({
      id: args.id,
    }).catch(error => errorHandler(error, loggers))
