  ...
  return async (args: input): Promise<returningSuccessObj<Model<foundationUser> | null>> => {
    ...
    if (!args.id) {
      return endMainFromError({
        ...
        errorIdentifier: "foundationUser_deactivateOne_error0001"
      })
    }
    ...
    if (!isIdStringFromUuid.result) {
      return endMainFromError({
        hint: "Datapoint 'id' is not UUID format.",
        errorIdentifier: "foundationUser_deactivateOne_error0002"
      })
    }
    ...
    if (!isIdValid.result) {
      return endMainFromError({
        ...
        errorIdentifier: "foundationUser_deactivateOne_error0003"
      })
    }
