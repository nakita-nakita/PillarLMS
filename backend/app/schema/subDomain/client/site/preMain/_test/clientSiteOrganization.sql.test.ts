...
import makeClientSiteOrganizationSql from "../clientSiteOrganization.sql";

jest.setTimeout(100000)

describe("test clientSiteOrganization.sql.js", () => {
  let d: dependencies;
  ...
  test("updateOne:can upsert record.", async () => {
    const organizationSql = makeClientSiteOrganizationSql(d)
  ...
  test("getOne: backkendSettingOrganization can get record.", async () => {
    const backendUserRequestLogic = makeClientSiteOrganizationSql(d)
