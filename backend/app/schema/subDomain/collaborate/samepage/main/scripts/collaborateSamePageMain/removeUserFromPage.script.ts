  ...
  return async (args: input): Promise<returningSuccessObj<SamePageObject>> => {
    ...
    if (!args.userId) {
      return endMainFromError({
        ...
        errorIdentifier: "collaborateSamePage_removeUserFromPage_error:0001"
      })
    }
    ...
    if (!isUserIdUuid.result) {
      return endMainFromError({
        ...
        errorIdentifier: "collaborateSamePage_removeUserFromPage_error:0002"
      })
    }
    ...
    if (!args.url) {
        return endMainFromError({
          hint: "'url' is missing.",
          errorIdentifier: "collaborateSamePage_removeUserFromPage_error:0003"
        })
      }
    ...
    const response = await samePage.removeUserFromPage({
      userId: args.userId,
      url: args.url,
