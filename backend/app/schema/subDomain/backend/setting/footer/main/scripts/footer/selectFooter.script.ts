...
export default function selectFooter(d: dependencies) {
    ...
    if (doesEntityExist.result) {
      ...
      switch (args.type) {
        case "BUILT_IN":
          ...
          const settingFooterBuiltInMain = makeBackendSettingFooterBuiltInMain(d)

          const builtInRecords = await settingFooterBuiltInMain.getMany()
      ...
      if (args.socketId) {
        entityRecord.data.sockets.map(s => {
          if (s.socketId !== args.socketId) {
            s.socket.emit("samedoc-footer-selection-change")
          }
        })
      }
