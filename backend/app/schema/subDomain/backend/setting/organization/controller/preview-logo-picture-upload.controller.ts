...
export default ({ app }) => {
  ...
  app.post('/backend/api/v1/logo/preview/file/', uploaderAuth.isAuthenticated, previewLogoUpload.single("file"), async (req, res) => {
    let picture
    const data: any = {}
    ...
    if (req.file?.filename) {
      ...
      picture = `/backend/api/v1/logo/preview/file/${year}/${month}/${req.file.filename}`
      data.picture = `/backend/api/v1/logo/preview/file/${year}/${month}/${req.file.filename}`
      ...
      await pictureSelection.uploadPicture({
        entity: "backendSettingOrganization",
        name: req.body.name,
        picture,
