...
export default function getManyWithPagination({ subDomainDb, errorHandler, transaction, loggers, }: d_sub) {
  ...
  return async (args: input): Promise<returningSuccessObj<findAndCountAll<backendSiteDesigner_pageTemplate>>> => {
    if (!args) {
      args = {
        q: undefined,
        page: 1,
    ...
    let data: findAndCountAll<backendSiteDesigner_pageTemplate> = await db.backendSiteDesigner_pageTemplate.findAndCountAll(search).catch(error => errorHandler(error, loggers)).catch(error => errorHandler(error, loggers))
    data.page = page + 1;
